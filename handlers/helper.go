...
func GetMap(uurl string, wait time.Duration) (map[string]*protobuf.DayWork, error) {
	//client := &http.Client{}
	client := &http.Client{}
	result := make(map[string]*protobuf.DayWork)
	...
	for i := start_ind + 1; i <= end_ind; {
		...
		for EndDay {
			...
			if hour == 3 {
				EndDay = false
				result[tmp] = &protobuf.DayWork{DayWorkload: tmp_map}
				cnt++
				i += 9
				break
			}
